    ...
    public class DebitRepositories : BaseRepository<Debit>, IDebitRepositories
    {
        ...
        public async Task<PaginatedResultReact<object>> GetObjectBaoCaoDoanhThuAsync(DebitDto DebitDto, int page, int pageSize, CancellationToken cancellationToken)
        {
            ...
            sql = $@"
              ...
              SELECT COALESCE(SUM(d.price), 0)+COALESCE(SUM(d.price_com), 0)+COALESCE(SUM(d.driver_fee),0) AS total_price FROM debits d WHERE d.type IN (0,1,2,3,4,8) AND (d.file_info_id IS NULL OR d.file_info_id = 0) AND d.deleted_at IS NULL";
            if (DebitDto.StorageId > 0)
            {
                sql += $@" AND d.storage_id = {DebitDto.StorageId}";
            }
            ...
            _results.Extra["cp_hasfile_results"] = cp_hasfile_results;
             ...
             sql = $@"
              ...
              SELECT  COALESCE(SUM(d.purchase_price), 0)+ COALESCE(SUM(d.purchase_com), 0) AS total_purchase_price FROM debits d WHERE d.type IN (0,1,2,3,4) AND (d.file_info_id IS NULL OR d.file_info_id = 0) AND d.deleted_at IS NULL";
            if (DebitDto.StorageId > 0)
            {
                sql += $@" AND d.storage_id = {DebitDto.StorageId}";
            }
        ...
        public async Task<PaginatedResultReact<object>> GetObjectDebitTongHopKHAsync(DebitDto DebitDto, int page, int pageSize, CancellationToken cancellationToken)
        {
            ...
            var sql = $@"
                      SELECT
                        ...
                        /* ===== DEBIT ===== */
                        CAST(SUM(ISNULL(d.price, 0)) AS INT) AS debit_price,
                        CAST(SUM(
                            ISNULL(d.price, 0) + ISNULL(d.price * (d.vat / 100.0), 0)
                        ) AS INT) AS debit_total,

                        /* ===== RECEIPT ===== */
                        CAST(SUM(ISNULL(rdt.amount, 0)) AS INT) AS receipt_amount,
                        CAST(SUM(ISNULL(rdt.amount * (rdt.vat / 100.0), 0)) AS INT) AS receipt_vat,
                        CAST(SUM(
                    ...
                    /* ===== RECEIPT (LEFT JOIN để giữ chưa thu) ===== */
                    LEFT JOIN receipt_details rdt 
                        ON rdt.debit_id = d.id
                        AND rdt.deleted_at IS NULL

                    LEFT JOIN receipts r 
                        ON r.id = rdt.receipt_id
                        AND r.deleted_at IS NULL";
                    if (DebitDto.FromDate.HasValue && DebitDto.ToDate.HasValue)
                    {
                        // Cộng thêm 1 ngày cho ToDate
                   ...
                   sql += $@" LEFT JOIN income_expense_categorys iecat
                        ON iecat.id = r.income_expense_category_id
                        AND iecat.type = 0
