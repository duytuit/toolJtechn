...
namespace WSChat.Backend.Application.Services
{
    public class MessageService : IMessageService
    {
        ...
        public IList<ChatPayloadDTO> HandleRequest(string userId, string message)
        {
            ...
            if (string.IsNullOrEmpty(objMessage.Chanel))
            {
                objMessage.Event = EventEnum.Error;
                objMessage.MessageText = "Trường Chanel là bắt buộc.";
                result.Add(objMessage.MessageToPayload(userId));
                return result;
            }
            ...
            switch (objMessage.Event)
            {
                ...
                case EventEnum.MessageAndUpdateUserRoom:
                    result = ProcessMessageAndUpdateUserRoom(userId, objMessage);
                    break;
        ...
        private IList<ChatPayloadDTO> ProcessMessageAndUpdateUserRoom(string userId, Message message)
        {
            IList<ChatPayloadDTO> result = new List<ChatPayloadDTO>();

            // kiểm tra xem có phòng chưa
            Room room = _roomService.CreateRoomV2(message.Chanel);

            // kiểm tra xem user có trong phòng chưa thì tạo

            ChatUser _chatUser = _userService.CreateUserV2(userId, userId, room.Id);

            // gửi tin nhắn cho tất cả user trong phòng

            message.User = GetUserById(userId);

            message = new MessagingHandler(_userService, _roomService).Handle(userId, message);

            // Composing message with target user informations
            if (message.Event != EventEnum.Error && !String.IsNullOrWhiteSpace(message.TargetUserId))
            {
        ...
        private IList<ChatPayloadDTO> ProcessRequestRegisterUser(string userId, Message message)
        {
            ...
            message = new RegisterUserHandler(_userService).Handle(userId, message);
        ...
        private IList<ChatPayloadDTO> ProcessRequestEnterRoom(string userId, Message message)
        {
            ...
            message = new EnterRoomHandler(_userService, _roomService).Handle(userId, message);
        ...
        private IList<ChatPayloadDTO> ProcessRequestCreateRoom(string userId, Message message)
        {
            ...
            message = new CreateRoomHandler(_userService, _roomService).Handle(userId, message);
