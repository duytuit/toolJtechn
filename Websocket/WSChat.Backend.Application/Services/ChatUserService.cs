...
namespace WSChat.Backend.Application.Services
{
    public class ChatUserService : IChatUserService
    {
        ...
        public ChatUser CreateUserV2(string id, string nickname, int idRoom)
        {
            ChatUser _chatUser = _users.FirstOrDefault(x => x.Value.Nickname == nickname).Value;
            if (!string.IsNullOrEmpty(_chatUser?.Nickname))
            {
                return _chatUser;
            }
            _users.TryAdd(id, new ChatUser(id, nickname));
            _chatUser = _users.FirstOrDefault(p => p.Value.Id == id).Value;
            ChatUser chatUserOld = _chatUser;
            _chatUser.IdActiveRoom = idRoom;
            _users.TryUpdate(_chatUser.Id, _chatUser, chatUserOld);
            return _chatUser;
        }
