...
namespace WindowsFormsApp5
{
    public partial class Form1 : Form
    {
        ...
        private void AddBarcode(Point location, string text = "1234567890", bool pureBarcode = true)
        {
            ...
            var pic = new PictureBox
            {
                ...
                Tag = new ElementData { Type = "Barcode", Text = text, PureBarcode = pureBarcode }
            };
            AddCommonHandlers(pic);
            designPanel.Controls.Add(pic);
        }
        ...
        {
            var writer = new BarcodeWriter { Format = BarcodeFormat.QR_CODE, Options = new ZXing.Common.EncodingOptions { Width = 150, Height = 150, Margin = 0, PureBarcode = true } };
            var pic = new PictureBox
            {
                Image = writer.Write(text),
                Location = location,
        ...
        private void AddCommonHandlers(Control ctrl)
        {
            ...
            ctrl.MouseMove += (s, e) =>
            {
                ...
                if (isResizing && activeCtrl == ctrl)
                {
                    ...
                    {
                        var writer = new BarcodeWriter { Options = new ZXing.Common.EncodingOptions { Width = pic.Width, Height = pic.Height, Margin = data.Type == "Barcode" ? 2 : 0, PureBarcode = data.Type == "Barcode" ? data.PureBarcode : true } };
        ...
        private void EditElement(Control ctrl)
        {
            ...
            if (data != null && data.Type == "Barcode")
            {
                using (var dlg = new Form { Width = 300, Height = 200, Text = "Chỉnh sửa nội dung" })
                {
                    var txt = new TextBox { Text = data.Text, Dock = DockStyle.Top };
                    var chkPure = new CheckBox { Text = "PureBarcode (ẩn text dưới mã vạch)", Checked = data.PureBarcode, Dock = DockStyle.Top };
                    var btn = new Button { Text = "OK", Dock = DockStyle.Bottom };
                    btn.Click += (s, e) =>
                    {
                        data.Text = txt.Text;
                        data.PureBarcode = chkPure.Checked;
                    ...
                    dlg.Controls.Add(chkPure);
                    dlg.Controls.Add(btn);
                    dlg.ShowDialog();
                }
            }
            if (data != null && data.Type == "QRCode")
            {
                    ...
                    btn.Click += (s, e) =>
                    {
                        ...
                        if (ctrl is PictureBox p)
                        {
                            var writer = new BarcodeWriter
                            {
                                ...
                                Options = new ZXing.Common.EncodingOptions { Width = p.Width, Height = p.Height, Margin = data.Type == "Barcode" ? 2 : 0, PureBarcode = true }
                            };
                            p.Image = writer.Write(data.Text);
                        }
                        dlg.Close();
                ...
                else if (ctrl is PictureBox pic)
                {
                    ...
                    if (data != null)
                        items.Add(new LayoutItem
                        {
                            ...
                            Width = pic.Width,
                            Height = pic.Height,
                            PureBarcode = data.Type == "Barcode" ? data.PureBarcode : (bool?)null
                        });
                }
            }
                ...
                if (ctrl is PictureBox pic && pic.Tag is ElementData data)
                {
                    var writer = new BarcodeWriter
                    {
                        ...
                        Options = new ZXing.Common.EncodingOptions
                        {
                            Width = pic.Width,
                            Height = pic.Height,
                            Margin = data.Type == "Barcode" ? 2 : 0,
                            PureBarcode = data.Type == "Barcode" ? data.PureBarcode : true
                        }
                    };
                    pic.Image = writer.Write(data.Text);
                }
        ...
        public class ElementData
        {
            ...
            public bool PureBarcode { get; set; } // Thêm dòng này
        }
