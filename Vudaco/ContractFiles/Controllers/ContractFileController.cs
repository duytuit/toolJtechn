...
namespace Vudaco.ContractFiles.Controllers
{
    ...
    public class ContractFileController : BaseApiController
    {
        ...
        [HttpGet("excel/GetObjectXuatHoaDonKHInDebitAsync")]
        public async Task<IActionResult> GetObjectXuatHoaDonKHInDebitAsync(CancellationToken cancellationToken, [FromQuery] int page = 1, int pageSize = 50, [FromQuery] DebitDto DebitDto = null)
        {
            try
                {
                    var result = await _repoDebit.GetObjectXuatHoaDonKHInDebitAsync(
                        DebitDto, page, pageSize, cancellationToken);
                    if (result?.Data == null || !result.Data.Any())
                        return ApiResponseResult<object>(false, "Không tìm thấy dữ liệu", null);
                    var data = result.Data.Select(x => (dynamic)x).ToList();
                        ...
                        if (DebitDto.Invoice == 1)
                        {
                            foreach (var item in group.Items)
                            {
                                ...
                                if(DebitDto.Version == 1)
                                {
                                    ...
                                    ws.Cell(row, 16).Value = "VND";
                                    ws.Cell(row, 20).Value = 8;
                                    ws.Cell(row, 21).Value = vat;
                                    ws.Cell(row, 22).Value = item.type == 1 ? "CƯỚC VẬN CHUYỂN " + (item.name ?? ""): "Phí dịch vụ giao nhận";
                                    ws.Cell(row, 24).Value = "chuyến";
                                    ws.Cell(row, 25).Value = 1;
                                    ws.Cell(row, 26).Value = price;
                                    ws.Cell(row, 29).Value = price;
                                    ws.Cell(row, 30).Value = 4;
                                    ws.Cell(row, 31).Value = 2;
                                    ws.Cell(row, 34).Value = item.vehicle_number ?? "";
                                }else{
